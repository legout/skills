---
name: make-release
description: Create and publish a patch, minor, or major release for Python/uv or Node projects. Use whenever the user asks to bump a version, update a changelog, tag a release, publish a GitHub Release, or publish a Python package to PyPI.
...
1. Require a clean Git worktree on the repository's default branch with an upstream. Fetch and ensure the branch can fast-forward cleanly. Never force-push.
2. Load `github`; verify `gh auth status` and the GitHub remote.
...
2. Draft one dedicated publish workflow that builds with `uv build`, passes the exact built artifacts between jobs, and publishes with `pypa/gh-action-pypi-publish@release/v1` on version tags. Do not write it before release-plan approval.
...
3. Treat the absence of a PyPI workflow plus an available local uv publishing credential as local mode. If still ambiguous on a later release, ask again.
...
2. Load `update-changelog` and collect notable user-facing changes since the latest release tag.
...
Run builds, not the full test suite unless repository instructions require more:

- Python: remove only ignored/untracked stale distribution artifacts, run `uv build`, run `uvx twine check dist/*`, and inspect wheel/sdist names plus embedded name/version metadata.
...
2. Load `commit` and create one release commit: `chore(release): <version>`. Never bypass commit hooks with `--no-verify`.
