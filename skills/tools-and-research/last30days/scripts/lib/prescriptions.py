...
_ENGINE = Path(__file__).resolve().parents[1] / "last30days.py"
ENGINE_CLI = f'"{sys.executable}" "{_ENGINE}"'
...
_YTDLP_BREW_INSTALL, _YTDLP_BREW_REINSTALL = health.static_prescription(
    "yt-dlp", "brew"
)
...
def lookup(source: str, failure: str) -> Prescription | None:
...
def _dependency_failure(probe: health.DependencyProbe) -> tuple[str, str] | None:
...
def for_dependency_probe(probe: health.DependencyProbe) -> Prescription | None:
