...
`uv_build` is a simple default for new pure-Python packages. Preserve an existing backend during migrations unless the user requests a backend change; compiled extensions, dynamic versioning, and specialized build steps may require another backend.
...
| --- | --- | --- |
| Application | Commit | Reproducible deployments |
| Library | Commit by default | Reproducible development and CI; downstream users still resolve from package metadata |
