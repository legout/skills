...
Prefer an already approved package manager, such as `brew install uv` or `pipx install uv`. Before using an official remote installer, confirm the current command in the uv documentation and obtain approval; do not pipe unreviewed downloads into a shell.
...
| `uv sync --locked` | Require `uv.lock` to be current before installing |
...
- Use `uv sync --locked` in CI so stale lockfiles fail instead of changing silently
- Use `uv cache clean` only when diagnosing cache problems and after confirming the deletion scope
